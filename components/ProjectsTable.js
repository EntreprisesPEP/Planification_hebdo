...
export default function ProjectsTable({ rows, editable, theme, onUpdate, board, emptyLabel }) {
  ...
  return (
    <div>
      {/* ---------- Vue tableau (ordinateur / iPad) ---------- */}
      <div className="scrollx desktop-table">
      ...
      <div className="mobile-cards">
        ...
        {visible.length === 0 && <div className="empty">{emptyLabel || 'Aucun projet.'}</div>}
        {visible.map((p) => (
          <ProjectCard key={p.id} project={p} editable={editable} theme={theme} onUpdate={onUpdate} board={board} />
        ))}
      </div>
    </div>
  );
